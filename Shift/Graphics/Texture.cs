...
namespace Shift.Graphics
{
    public class Texture : IDisposable
    {
        ...
        private void LoadBitmap(string ResourceFile, bool FlipY = true)
        {
            ...
            using (var image = System.Drawing.Image.FromFile(Filename))
            {
                ...
                var BitmapImage = (System.Drawing.Bitmap)System.Drawing.Bitmap.FromFile(Filename);
            ...
            if (FlipY) BitmapImage.RotateFlip(System.Drawing.RotateFlipType.RotateNoneFlipY);     // bitmaps read from bottom up, so flip it
            this.Size = new Point(BitmapImage.Size.Width, BitmapImage.Size.Height);

            // must be Format32bppArgb file format, so convert it if it isn't in that format
            System.Drawing.Imaging.BitmapData bitmapData = BitmapImage.LockBits(new Rectangle(0, 0, BitmapImage.Width, BitmapImage.Height), System.Drawing.Imaging.ImageLockMode.ReadOnly, System.Drawing.Imaging.PixelFormat.Format32bppArgb);
            ...
            Gl.TexImage2D(TextureTarget, 0, (int)PixelInternalFormat.Rgba8, BitmapImage.Width, BitmapImage.Height, 0, OpenGL.PixelFormat.Bgra, OpenGL.PixelType.UnsignedByte, bitmapData.Scan0);
            Gl.TexParameterI(TextureTarget, TextureParameterName.TextureMagFilter, new int[] { (int)TextureParameter.Nearest });
            Gl.TexParameterI(TextureTarget, TextureParameterName.TextureMinFilter, new int[] { (int)TextureParameter.Nearest }); //(int)TextureParam.Linear);   // Linear filter
        ...
        private void LoadDDS(string ResourceFile)
        {
            using (BinaryReader stream = new BinaryReader(new FileStream(ResourceFile, FileMode.Open)))
            {
                ...
                this.Size = new Point((int)imageData.Width, (int)imageData.Height);
                ...
                try
                {
                    ...
                    Gl.TexParameterI(TextureTarget, TextureParameterName.TextureMinFilter, new int[] { (int)TextureParameter.Linear });
                    Gl.TexParameterI(TextureTarget, TextureParameterName.TextureMagFilter, new int[] { (int)TextureParameter.Linear });
                    ...
                    for (int i = 0; i < (imageData.MipmapCount == 0 ? 1 : imageData.MipmapCount); ++i)
                    {
                        ...
                        if (compressed)
                        {
                            ...
                            Gl.CompressedTexImage2D(TextureTarget, i, (int)format, nWidth, nHeight, 0, nSize, (IntPtr)(pinned.AddrOfPinnedObject().ToInt64() + nOffset));
                        }
                        else
                        {
                            nSize = nWidth * nHeight * 4 * ((int)imageData.PixelFormat.RGBBitCount / 8);
                            Gl.TexImage2D(TextureTarget, i, (int)format, nWidth, nHeight, 0, OpenGL.PixelFormat.Bgra, PixelType.UnsignedShort4444, (IntPtr)(pinned.AddrOfPinnedObject().ToInt64() + nOffset));
                        }
